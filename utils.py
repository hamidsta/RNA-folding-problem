...
def plot_script_2(nested_dict,direct):
    ''' Take nested dict and directory as input, will plot value of each key
    ( 1 key of nested dict correspond to one combination of nucleotide )
...
def save_file ( directory,parent,file):
    ''' #Take nested dict and directory as input,
         will create 10 files corresponding to each key of dict'''
    dir = directory
...
def all_in_one(directory):
    ''' all_in_one will take all function and compute a pseudo score given the input.
     take into account pair in same chain and  if file is RNA or not'''
    d = dict()                                                          # create dict
    d_no_round=dict()
...
def readFasta(seqFile):
    ''' read file ( not fasta file, but you get it  ) .
    PDB file will be read and ATOM line will be store inside nested list'''
...
def score_pseudo_energy(observed, reference):
    '''pseudo energy score compute log frequence of both frequency ,
    take into account 0 division and log error '''
...
def new_pseudo_linear_interpolation(d_no_round,pseudo_NRJ):
    '''# part 3 , linear interpolation ,
     need no round dict and pseudo NRJ'''
    ...
    for new_key, value in d_no_round.items():
        ...
        for i in range ( len (value)):
            x = value[i]
            x1=math.floor(value[i])  # tail and floor of value
